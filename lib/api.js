...
export async function getAllExperiencesWithSlug() {
  const entries = await fetchGraphQL(
    `query {
      postCollection(where: { slug_exists: true }, order: startDate_DESC) {
        items {
          ${EXPERIENCE_GRAPHQL_FIELDS}
        }
      }
...
export async function getAllExperiencessForHome(preview) {
  const entries = await fetchGraphQL(
    `query {
      postCollection(order: startDate_DESC, preview: ${preview ? 'true' : 'false'}) {
        items {
          ${EXPERIENCE_GRAPHQL_FIELDS}
        }
      }
...
export async function getExperienceAndMoreExperiencess(slug, preview) {
  ...
  const entries = await fetchGraphQL(
    `query {
      postCollection(where: { slug_not_in: "${slug}" }, order: startDate_DESC, preview: ${preview ? 'true' : 'false'
    }, limit: 2) {
        items {
          ${EXPERIENCE_GRAPHQL_FIELDS}
        }
